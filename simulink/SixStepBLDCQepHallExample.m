...
% <docid:mcb_ref#mw_e3591137-bd90-4528-bd5b-223b39a8ede9 Six Step
% Commutation>.
% 
% The six-step commutation algorithm requires a Hall sequence or a rotor
% position feedback value (which is obtained from either a quadrature
...
% the controller uses to determine the 60-degree sector where the rotor is present.
% 
...
% 
% *Note:* The Hall sequence can vary. Use the example
% <docid:mcb_gs#mw_ac526d91-7b0a-429b-beed-b32ed16ccd64 Hall Sensor
% Sequence Calibration of BLDC Motor> to determine the Hall sequence.
% 
% The example supports simulation. You can simulate a plant model
% containing a three-phase switching inverter along with wye-wound BLDC from
% Simscape(TM) Electrical(TM).
% 
...
% 
% * <matlab:openExample('mcb/SixStepBLDCQepHallExample','supportingFile','mcb_bldc_sixstep_f28069mLaunchPad.slx') mcb_bldc_sixstep_f28069mLaunchPad>
% 
% * <matlab:openExample('mcb/SixStepBLDCQepHallExample','supportingFile','mcb_bldc_sixstep_f28379d.slx') mcb_bldc_sixstep_f28379d>
% 
...
% We provide default motor parameters with the Simulink(R) model that you can replace with values from either the motor datasheet or other sources.
% 
...
% 
% *2.* Select either the QEP or the Hall Speed_Feedback radio button in the
% model.
% 
...
% This section shows you how to generate code and run the six-step commutation algorithm on
% the target hardware.
% 
...
% 
% * LAUNCHXL-F28069M controller + BOOSTXL-DRV8305 inverter: <matlab:openExample('mcb/SixStepBLDCQepHallExample','supportingFile','mcb_bldc_sixstep_f28069mLaunchPad.slx') mcb_bldc_sixstep_f28069mLaunchPad>
% 
% * LAUNCHXL-F28379D controller + BOOSTXL-DRV8305 inverter: <matlab:openExample('mcb/SixStepBLDCQepHallExample','supportingFile','mcb_bldc_sixstep_f28379d.slx') mcb_bldc_sixstep_f28379d>
% 
...
% 
% *4.* If you are using a quadrature encoder, compute the quadrature
% encoder index offset value and update it in the |bldc.PositionOffset| variable available in the model initialization
% script associated with the target model. For instructions, see
% <docid:mcb_gs#mw_52571b8e-639e-4a24-a8bf-b644eb78edc1 Quadrature Encoder
% Offset Calibration for PMSM Motor>.
% 
% *5.* If you are using a Hall sensor, compute the Hall sequence value and
% update it in the |bldc.hallsequence| variable available in the model initialization script associated with the target
...
% 
% *8.* Load a sample program to CPU2 of LAUNCHXL-F28379D. For example, you
% can use the program that operates the CPU2 blue LED by using GPIO31
% (c28379D_cpu2_blink.slx), and ensure that CPU2 is not mistakenly
...
% 
% *9.* Click *Build, Deploy & Start* on the *Hardware* tab to deploy the
% target model to the hardware and load the variables from the target model
% to the base workspace.
% 
% *10.* Click the <matlab:openExample('mcb/SixStepBLDCQepHallExample','supportingFile','mcb_bldc_host_model_f28379d.slx') host model> hyperlink in the target model to open the
...
% 
% *11.* In the model initialization script associated with the target
% model, specify the communication port using the variable
% _target.comport_. This variable updates the *Port* parameter of the Host
% Serial Setup, Host Serial Receive, and Host Serial Transmit blocks available in the
% host model.
%  
